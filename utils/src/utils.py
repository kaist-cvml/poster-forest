...
def _soffice_env():
    # Workaround: /usr/local/lib/libstdc++.so.6 lacks GLIBCXX_3.4.30 required by libreoffice.
    # Preload the system libstdc++ which ships a compatible version.
    env = os.environ.copy()
    good_libstdcpp = "/usr/lib/x86_64-linux-gnu/libstdc++.so.6"
    if os.path.exists(good_libstdcpp):
        existing = env.get("LD_PRELOAD", "")
        env["LD_PRELOAD"] = good_libstdcpp + (":" + existing if existing else "")
    return env
...
def get_json_from_response(raw_response: str):
    if '</think>' in raw_response:
        raw_response =  raw_response[raw_response.rfind("</think>")+8:]
        # print("++++")
        # print(raw_response)
        # print("++++")
...
def ppt_to_images(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='png'):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        subprocess.run(command_list, check=True, stdout=subprocess.DEVNULL, env=_soffice_env())
...
@tenacity
def ppt_to_images_with_title(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='png', title="", stem=None):
    assert pexists(file), f"File {file} does not exist"
    if pexists(output_dir) and warning:
        print(f"ppt2images: {output_dir} already exists")
    os.makedirs(output_dir, exist_ok=True)
    basename = stem if stem is not None else f"poster_{title}"
    with tempfile.TemporaryDirectory() as temp_dir:
        command_list = [
            "soffice",
            "--headless",
        ...
        for f in os.listdir(temp_dir):
            ...
            temp_pdf = pjoin(temp_dir, f)
            images = convert_from_path(temp_pdf, dpi=dpi)
            ext = 'png' if output_type == 'png' else 'jpg'
            fmt = 'PNG' if output_type == 'png' else 'JPEG'
            for i, img in enumerate(images):
                img.save(pjoin(output_dir, f"{basename}.{ext}"), fmt)
                if output_type == 'png':
                    print(f"Saved image {i + 1} as PNG in {output_dir}")

            path = pjoin(output_dir, f"{basename}.{ext}")
            print(f"Converted {f} to images in {path}")
            return
